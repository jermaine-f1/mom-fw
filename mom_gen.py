...
# Overlay regions are tag-only universes — symbols here are also classified under
# a primary region (US/EM/Commodities/…). Listed in one place so dedup, the
# analyzer's unmapped-region check, and the CIO sub-tabs stay in sync.
...
def load_etf_universe(csv_path):
    ...
    # Push overlay universes to the bottom so drop_duplicates keeps the primary
    # region as the canonical row for tickers that appear in multiple universes.
    df = df.sort_values(
        by='Region',
...
def generate_html(etf_data, correlations, generation_date):
    ...
    overlay_regions_json = json.dumps(list(OVERLAY_REGIONS))
...
<body class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-slate-100 p-4 md:p-6">
  ...
  <script>
    ...
    const OVERLAY_REGIONS = new Set({overlay_regions_json});
    ...
    function renderAnalyzer(holdings, unrecognized, filteredSellRows) {{
      ...
      holdings.forEach(h => {{
        ...
        if (c && !OVERLAY_REGIONS.has(c) && !(c in COUNTRY_TO_CIO_ID)) {{
    ...
    // Per-country max% is driven by the Settings > CIO asset-class constraints
    // (cioConstraints). Countries without a mapping have no configured max.
    const COUNTRY_TO_CIO_ID = {{
      'Emerging Markets': 'em',
      'India': 'em',
    ...
    function getAssetClassMaxPct(country) {{
      const id = COUNTRY_TO_CIO_ID[country];
      if (!id) return null;
    ...
    function renderAnalyzerActions(holdings, totalValue) {{
      ...
      holdings.forEach(h => {{
        ...
        if (maxPct != null && countryPct > maxPct) {{
      ...
      const buyCandidates = rankedETFs.filter(e => {{
        ...
        const currentPct = totalValue > 0 ? ((countryWts[c] || 0) / totalValue * 100) : 0;
        const maxPct = getAssetClassMaxPct(c);
        return maxPct == null || currentPct < maxPct;
      }});
